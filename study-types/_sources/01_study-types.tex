...
In empirical studies, LLMs can act as raters (also known as judges in the literature) to evaluate the quality or other properties of software artifacts.% such as code, documentation, and design patterns.
For instance, LLMs can be trained to assess code readability, adherence to coding standards, or the quality of comments. 
\emph{Rating} is distinct from the more qualitative task of assigning a code or label (see \ref{sec:annotate}) to (typically) unstructured text.
It is also distinct from the software engineering tasks of \textbf{using} LLMs, as we discuss in \ref{sec:llm4se} to e.g. understand if LLMs can find ambiguity in requirements~\cite{10.1145/3510003.3510157}, which is then compared to a human-created gold standard.
...
\subsubsection{Example(s)}
Lubos et al.~\cite{lubosRE24} leveraged Llama-2\footnote{\url{https://www.llama.com/llama2/}} to evaluate the quality of software requirements statements. 
...
Evaluate whether the following requirement is \{quality_characteristic\}. \{quality_ characteristic\} means: \{quality_characteristic_explanation\}\\
...
Research is ongoing as to how suitable LLMs are as standalone raters. Questions around bias, accuracy, and trust remain~\cite{bavaresco2024llmsinsteadhumanjudges}. There is reason for concern about LLMs judging student assignments or doing peer review of scientific papers~\cite{zhou-etal-2024-llm}. Even beyond questions of technical capacity, ethical questions remain as well. Involving a human in the rating loop---for example, to contextualize the scoring---is one approach~\cite{panHumanCenteredDesignRecommendations2024}.
