...
This might include, for example, evaluating the code smells in a piece of code. 
...
It is also distinct from the software engineering tasks of \textbf{using} LLMs, as we discuss in \ref{sec:llm4se} to e.g. understand if a requirement is ambiguous~\cite{10.1145/3510003.3510157}.
...
%\todo{Examples of such studies in software engineering include...}
...
LLMs can also be much more efficient than the equivalent human approach. Within the limits of the input context size, 
...
Avoid the temptation to assign objectivity to an LLM; evidence shows different LLMs behave differently. 

Rating large numbers of items---for example, the thousands of bugs big open-source projects deal with---comes with costs in clock time, compute time, and sustainability.

Research is ongoing as to how suitable LLMs are as standalone raters. Questions around bias, accuracy, and trust remain~\cite{bavaresco2024llmsinsteadhumanjudges}. There is reason for concern about LLMs judging student assignments or doing peer review of scientific papers~\cite{zhou-etal-2024-llm} (beyond the technical capacity, ethical questions remain as well). Involving a human in the rating loop---for example, to contextualize the scoring---is one approach~\cite{panHumanCenteredDesignRecommendations2024}.
