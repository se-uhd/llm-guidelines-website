...
\comment{I would clarify this better, so that no one thinks subjects are annotators or raters, etc. "In empirical research, 'subjects' are participants from whom data is collected. When using LLMs as subjects, the models generate responses that simulate human participants in controlled experiments, allowing researchers to collect data that approximates human reactions without actual human involvement."}
...
\comment{I confess I didn't know about these four criteria. Is it maybe worth mentioning them in the guidelines, besides providing the reference? If I understood them correctly, I would add something along these lines: "For indistinguishability, LLM-generated code reviews should be comparable to those from real developers; for consistency, if simulating a senior Python developer, responses should reflect that expertise level consistently. etc..."}
...
\comment{I wonder if we need to focus on other aspects besides demographic biases. Maybe we add a discussion of construct validity (do LLM-simulated subjects actually measure what we intend?), internal validity (can we draw causal conclusions?), and external validity (do findings generalize to real developers?).}

\comment{Maybe the text here can also address that researchers must validate that LLM simulations actually represent realistic human behaviors? "Researchers should validate LLM-generated responses against baseline human data before full deployment. For example, in a pilot study, collect responses from both real developers and LLM-simulated developers on the same tasks, then analyze similarities and differences in response patterns before scaling up with LLM subjects only."}

\comment{I know we cover declaration of usage and role in the guidelines, but do we need extra care regarding ethical concerns here? So maybe to make the guidelines more practical, we may want to add guidance on ethical disclosure requirements. Something like this? "Researchers must  disclose that LLMs were used as experiment subjects instead of humans. This imjplies explicitly stating this in methodology sections and discussing potential ethical implications of replacing human participants with AI simulations, particularly for studies that might inform human-centered design decisions (seems relevant for the CPH workshop :-).}
