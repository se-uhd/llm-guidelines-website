...
LLMs can be used as ``participants'' in various types of empirical studies, enabling researchers to simulate human participants.
...
From an engineering standpoint, developing LLM-based tools can be more straightforward, as researchers can leverage LLMs as black boxes to perform a wide range of tasks without building ad hoc solutions. At the same time, they can fine-tune open-source LLMs for specific tasks—provided that suitable datasets are available—either to adhere to open science principles or to develop more tailored tools.

\subsubsection{Challenges}

The non-determinism of generative AI models—although potentially mitigated through prompt engineering—poses a significant challenge in both the development and evaluation of tools integrating GenAI. 

In terms of research, evaluating the effectiveness of LLM-based tools can be challenging since their non-determinism can lead to different outputs for the same input, making it difficult to assess their performance consistently.
Moreover, when using non-open-source LLMs, researchers may face challenges related to the availability of the model, its versioning, and the reproducibility of results, as well as convincing practitioners to adopt the tool for ethical and legal reasons.

From a development perspective, while open models are accessible, the most performant ones require substantial hardware resources that are not yet widely available. Resorting to cloud-based APIs using non-open models or relying on third-party providers for hosting, while seemingly a solution, introduces new concerns related to data privacy and security.
