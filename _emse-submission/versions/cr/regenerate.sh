...
SUBMISSION_DIR="$(cd "$CR_DIR/../.." && pwd)"
RESPONSE_DIR="$SUBMISSION_DIR/reviews-and-response"

# The paper sources live in the sibling submodule of the website repo.
PAPER_DIR="$(cd "$SUBMISSION_DIR/../llm-guidelines-paper" 2>/dev/null && pwd)" || {
  echo "ERROR: paper submodule missing; run 'git submodule update --init' in the website repo." >&2
  exit 1
}
