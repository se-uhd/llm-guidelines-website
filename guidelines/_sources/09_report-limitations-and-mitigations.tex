...
\comment{I've replaced all occurrences of " \~" with "\~", as "\~" already adds a space. With " \~", all references had two spaces before the reference number.}
...
\begin{itemize}
  ...
  \comment{This might be slightly pedantic, but we may consider rephrasing "disable non-determinism" to something more akin to "reduce output variability" or "increase determinism". As from my understanding, even with a temperature set to 0, in practice, the LLM will not always be 100 \% deterministic.}
\end{itemize}
...
\comment{I've noticed in this section and in some of the other sections, that some of the guidelines only apply in specific cases, or that there is a choice between two or more guidelines depending on the type of study. This could potentially be confusing, as some readers might expect that this information would be in the "study types" section, with a description of which types of studies the guidelines apply to. However, as we only differentiate between the study types described in the "study types" section, the fine-grained information mentioned here would not fit there. I did not make any changes regarding this, as I'm unsure if there is a better way to notify readers of small and fine-grained conditions for guidelines. This could be a topic of discussion for one of the meetings.}
...
\comment{I've slightly changed the formatting here, moving the unconditional guidelines to the top and the conditional ones to the bottom.}
...
\begin{itemize}
  ...
  \item \mustnot leak their fine-tune or evaluation datasets into the improvement process of the LLM \comment{We could consider decoupling the justification of the guidelines and the actual guideline a bit more, as the actual recommendation technically ends at this point and the rest of the bullet point is an explanation as to why we should have this guideline.}(e.g., OpenAI's functionality called ``Improve the model for everyone``), as this potentially impacts further evaluation rounds (e.g., think about pass@k), and exacerbates the issue of reproducibility.
...
\begin{itemize}
  ...
  \comment{I added a small bit of additional context to the example of multi-agent-systems, check if you think the guideline is more clear like this.}
  ...
  \comment{What is meant by "sample" here? Consider adding a bit more details.}
\end{itemize}
...
\begin{itemize}
  \item \should clarify and state all relevant requirements, and employ and report the metrics to measure the satisfaction of the requirements (e.g., test-case success rate) and \comment{I've added a clarification of "their satisfaction" here. It makes the guidelines a bit less nice to read, but I think it's more clear like this.}
...
In the domain of code generation, Coignion et al.~\cite{DBLP:conf/ease/CoignionQR24} evaluated the performance of LLMs to produce leet code. To mitigate the issue of inter-dataset duplication, they only used leet code problems published after 2023-01-01\comment{I switched this date to ISO format.}, reducing the likelihood of LLMs having seen those problems before. Further, they discuss the performance differences of LLMs on different datasets in light of potential inter-dataset duplication.
...
\comment{I added a small section on the advantages of ensuring reproducibility.}
