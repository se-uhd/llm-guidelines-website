...
\begin{quote}
\underline{tl;dr:} Researchers \must justify benchmark and metric choices in the \paper and \should summarize benchmark structure, task types, and limitations. Where possible, traditional (non-LLM) baselines \should be used for comparison. Researchers \should use established metrics and \must explain why they are suitable for the specific study. Due to LLM non-determinism, experiments \should be repeated; the result distribution \should then be reported using descriptive statistics.
\end{quote}
