...
\begin{quote}
\underline{tl;dr:} Researchers \must use human validation for LLM outputs when no reference dataset exists or when automated metrics are insufficient. They \must define the measured construct and validation methods in the \paper. Researchers \should plan validation early, use validated instruments, reference established comparison models, and include materials in the \supplementarymaterial. When aggregating judgments, methods and rationale \should be reported, and inter-rater agreement \should be assessed. Confounding factors \should be controlled (e.g., via stratified sampling), and power analysis \should be conducted to ensure statistical robustness. LLM-generated annotations \may be used if supported by systematic agreement/confidence methods.
\end{quote}
