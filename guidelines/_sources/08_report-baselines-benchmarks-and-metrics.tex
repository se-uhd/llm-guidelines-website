...
Benchmarks, baselines, and metrics play a significant role in assessing the effectiveness of LLMs or LLM-based tools for working on specific tasks. \comment{Contextualize this to SE. I would avoid talking about generic LLM since assessign their effectiveness in other tasks that are not SE-related is out of scope. Similarly, I would explicitly mention ``SE-specific tasks''}
When selecting benchmarks, it is important to understand the contained data, task, and solutions, because this determines what exactly the benchmark assesses. \comment{Clarify what is a ``solution'' in the context of a benchmark; is it the same as ground truth?}
We recommend researchers to briefly summarize the select benchmark and why they consider it suitable for their study.
\comment{Here I would report the minimum information that should go in the summary. Is it the same as above (data, task, etc)?}
If multiple benchmark exist for the same task, the goal should always be to compare performance between benchmarks.
\comment{I liked a lot this idea of comparison across benchmarks, although the wording is a bit strong (``always''). Perhaps, this can rephrased to indicate that if a comparison is not done, this must be clearly justified.}
...
\comment{Representativeness is also a very sensible aspect. I am just unsure how this example relates to the previous statement about comparison across benchmarks. Maybe it is just a matter of phrasing but it seems these are two distinct concepts.}
...
To make such comparisons between traditional and LLM-based approaches, or comparison between LLM-based tools based on a benchmark, researchers need to carefully select suitable metrics.
...
\comment{Notice that this is mixing parameters for inference (temperature) with ones used for evaluation (top-k).This may be confusing for the readers.}
...
\comment{I like this paragraph. I suggest using it at the beginning of the section rather than here.}
...
For scenarios where $k>1$ is reasonable to include human-in-the-loop workflows or reranking approaches.
