...
\begin{quote}
\underline{tl;dr:} Researchers \must report study limitations, including non-determinism, lack of generalizability, and potential data leakage. The \paper \must specify whether generalization across LLMs or time was assessed, and discuss known model/version drift. Authors \must describe measurement constructs and methods, disclose any data leakage risks, and avoid leaking evaluation data into LLM improvement pipelines. They \must provide model outputs, discuss sensitive data handling (incl. GDPR and ethics approvals), and justify LLM usage in light of its resource demands.
\end{quote}
