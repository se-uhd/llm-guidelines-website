...
If one uses an LLM locally, e.g. llama one could argue that restarting it always and always providing the same context might result in deterministic outcomes. However, this is not the case for all of the models (see discussion about [discussion about GPT 4](https://152334h.github.io/blog/non-determinism-in-gpt-4/)). Moreover, even if a given locally run, open LLM might be deterministic, there is no guarantee that the specific version that was used in the study in the first place will be available for the long term. 
